...
def fitness_func(f_missile, distance):
    ...
    # if height - f_missile.y - 10 > 0:
    #     curr_fit = 1 / (distance + 1) * (height - f_missile.y - 10) / 10
    # else:
    #     curr_fit = 0
    if (height - f_missile.y - 10) < 200:
        curr_fit = (height - f_missile.y - 10) * 1 / (distance + 1)
    else:
        curr_fit = 200 * 1 / (distance + 1)
...
def game(genomes, config):
    ...
    while running:
        ...
        for x, (e_missile, f_missile) in enumerate(zip(enemy_missile_list, friend_missile_list)):
            ...
            if output[0] > 0.5:
                friend_missile_list[x].up_pressed = True
            else:
                friend_missile_list[x].up_pressed = False

            if output[1] > 0.5:
                friend_missile_list[x].down_pressed = True
            else:
                friend_missile_list[x].down_pressed = False

            if output[2] > 0.5:
                friend_missile_list[x].left_pressed = True
            else:
                friend_missile_list[x].left_pressed = False

            if output[3] > 0.5:
                friend_missile_list[x].right_pressed = True
            else:
                friend_missile_list[x].right_pressed = False
            ...
            if f_missile.y > height - 10 + 100:
                f_missile.destroyed = True
            if distance < 5 and e_missile.y > height - 50:
                e_missile.destroyed = True
                f_missile.destroyed = True
